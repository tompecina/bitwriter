...
public class SeqElement extends ParsedElement {
  ...
  // static logger
  private static final Logger log = Logger.getLogger(SeqElement.class.getName());
  ...
  private void process() throws ProcessorException, IOException {
    ...
    if (element.getTagName().equals("stream")) {
      processor.getControlledOutputStream().setDiscard(extractBooleanAttribute(
          element, "discard", false, processor.getScriptProcessor()));
      processor.getOutStream().setWidthOut(extractIntegerAttribute(
          element, "width-out", 1, null, 8, processor.getScriptProcessor()));
      processor.getOutAggregateStream().setWidthOutAggregate(extractIntegerAttribute(
          element, "width-aggregate-out", 1, null, 8, processor.getScriptProcessor()));
      processor.getOutAggregateStream().setEndiannessOut(Stream.Endianness.valueOf(extractStringArrayAttribute(
          element, "endianness-out", ENDIANESS_TYPES, "big", processor.getScriptProcessor()).toUpperCase()));
      processor.getBitStream().setReflectOut(extractBooleanAttribute(
          element, "reflect-out", false, processor.getScriptProcessor()));
      final int widthIn = extractIntegerAttribute(element, "width-in", 1, null, 8, processor.getScriptProcessor());
      processor.getInAggregateStream().setWidthInAggregate(extractIntegerAttribute(
          element, "width-aggregate-in", 1, null, widthIn, processor.getScriptProcessor()));
      processor.getInStream().setWidthIn(widthIn);
      processor.getInAggregateStream().setReflectIn(extractBooleanAttribute(
          element, "reflect-in", false, processor.getScriptProcessor()));
      processor.getInStream().setEndiannessIn(Stream.Endianness.valueOf(extractStringArrayAttribute(
          element, "endianness-in", ENDIANESS_TYPES, "big", processor.getScriptProcessor()).toUpperCase()));
    }
      ...
      for (Node content : children(element)) {
        if (content instanceof Text) {
          ...
          for (String split : new ScriptLine(trimmedText)) {
            final BigInteger value = processor.getScriptProcessor().evalAsBigInteger(split);
            if (value != null) {
              write(value);
            }
          }
   ...
   */
  public SeqElement(final InputTreeProcessor processor, final Element element) throws ProcessorException, IOException {
