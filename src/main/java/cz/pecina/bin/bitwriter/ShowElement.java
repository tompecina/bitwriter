...
public class ShowElement extends VariableElement {
  ...
  // static logger
  private static final Logger log = Logger.getLogger(ShowElement.class.getName());
  ...
  private void process() throws ProcessorException, IOException {
    ...
    if ((variable != null) && (value != null)) {
      throw new ProcessorException("Error in input file, 'name' and 'value' are incompatible in <show> element");
    }
    if ((variable == null) && (value == null)) {
      throw new ProcessorException("Error in input file, either 'name' or 'value' must be specified in <show> element");
    }
    if (variable != null) {
    ...
    if (variable != null) {
      processor.getStderr().println(String.format(
          "%s: %s (%s)", variable.getName(), value.toString(), Util.bigIntegerToString(value)));
    } else {
      processor.getStderr().println(String.format("%s (%s)", value.toString(), Util.bigIntegerToString(value)));
    }
   ...
   */
  public ShowElement(final InputTreeProcessor processor, final Element element) throws ProcessorException, IOException {
