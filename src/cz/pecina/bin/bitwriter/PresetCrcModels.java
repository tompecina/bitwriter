...
public class PresetCrcModels extends ArrayList<CrcModel> {
  ...
  private void process(final InputStream inputStream
	...
	if (notation == null) {
	  throw new PresetCrcModelsException(
	    "Notation must be specified in preset CRC models file");
	}
	int width = 0;
	...
	final Element checkElement =
	  (Element)(modelElement.getElementsByTagName("check").item(0));
	...
	if (model.hasCheck()) {
	  ...
	  if (c.getRegister().equals(model.getCheck())) {
	    ...
	    throw new PresetCrcModelsException("Model '" + model.getId() +
