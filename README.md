...
The listing contains the following fields for each model:
  ...
  RefIn:    reflect-in parameter, "F" stands for "false, "T" for true; if set,
            bits are processed by the CRC shift register in reverse order,
            i.e., LSB (bit 0) is read first and MSB (bit 7) last
...
separate unit, i.e., the concatenation takes place on the binary output level.
If the XML prolog is specified, the encoding (charset) is set to the default
...
Bitwriter reads and processes only well-formed and valid XML data in any
encoding recognized by the JRE (Java Runtime Environment).  The format of the
XML data is straightforward and easy to understand, with no irregularities or
idiosyncrasies.  It reminds more of a scripting or programming language than a
data XML file.
...
"0xff", or a script invocation, enclosed in double braces, e.g., "{{ i++ }}".


Bitwriter understands the following literal data:

Numbers: An integer can be expressed in hexadecimal, decimal, octal or binary
representation of any length.  Hexadecimal numbers must be preceded by 0x or
0X, binary numbers by 0b or 0B, octals must have a leading 0.  All numbers are
internally represented as Java's BigInteger objects so there is no limit as to
their size.

Boolean values: The format of XML Schema type xs:boolean is accepted, i.e.,
"true"/"false" or "1"/"0".
...
the string is the output of a script, e.g., {{ 'little' }}; without the
quotes, the script engine would output the value of the variable named
"little" rather than the string "little".
...
Any elements allowed on the outer level, i.e., <stream>, <loop>, <script> and
...
applied to the input data (i.e., the data model).  It can only appear on the
...
stream is repeated.  The repetition occurs on the semantic level, i.e., the
...
If no "width-aggregate-in" is specified, it also sets the width of the
...
"endianness-out" -- Optional, values like for "endianness-in", default
 "little".  The output endianness mode, determining the manner in which the
 output stream is formed from the aggreagate output.

"reflect-in" -- Optional, Boolean, default "false".  It determines the order
in which bits enter the bit stream.  If "true", the bit order is reversed,
i.e., LSB comes in the first, MSB the last.

"reflect-out" -- Optional, Boolean, default "false".  It determines the order
in which bits leave the bit stream.  If "true", bit order is reversed, i.e.,
LSB comes out the first, MSB the last.
...
e.g., variables are updated and processed, scripts are evaluated, etc.  In
...
All elements allowed on the outer level, i.e., outside a stream, namely
...
just like in a <stream>, i.e., no radix conversion is performed.
...
Octal numbers, including scripts, <flush> and <script>.
...
numeric value of the input stream, and it is up to the user to provide for
appropriate storage.
...
attributes.  Special characters can be expressed using XML entities, e.g.,
"&#7;".
...
"charset" -- Optional, default is the default encoding (charset) of the
operating system.  The charset (character set) used for converting the text
string to bytes.  All charsets recognized by Java are accepted, plus "raw",
which means that the code-point number of the character is used rather then
its -- possibly multi-byte -- representation.
...
downstream direction, i.e., "on-input-stream" first and "on-output-stream"
last.

"on-stream-in" -- Optional, integer.  A value to which the variable is set on
any write to input stream.  Typically a script, evaluation is deferred until
...
delete the string, an empty string, i.e., "", must be spefically indicated as
the attribute value.
...
The variable is released, i.e., all its scripts are reset and it is set to
normal mode.  The value in preserved.  The variable must exist, or an error is
reported.  This is practical for situations when a calculator, e.g., a CRC, is
...
"type" -- Optional, default "stream-in".  Destination stream for the data.
...
is divided by x, i.e., its representation is shifted one bit to the right.
...
reversed, i.e., the LSB of the register is presented as the MSB.
...
selection depends on the JRE in which the application is run, but all of the
common digest types, such as "MD5", "SHA-1" or "SHA-256", are supported.
Please refer to your current JRE documentation for a complete and current
list.
...
repetitions, different data will be included.  This can be useful, e.g., for
...
data, i.e., to split it into bytes according to the required output
endianness.
...
- All Bitwriter variables, including calculators, are set, using the same
  identifiers as in XML, e.g., if the XML file initialized the variable
  "total", it will be directly accessible from within the script as the
  JavaScript variable "total".  The value of the variable is a BitInteger
  object, and it is initialized as such, but the scripting language will
  convert the value to a number whenever needed.  This automatism is a very
  useful feature: it means that in most cases, the user does not have to
  bother about the internal representation of the variables and he/she can use
  them directly in arithmetic operations and function calls.  One only needs
  to access the variables as BigInteger's when they are wider than what ECMA
  script's universal numeric types can handle (which is something like 53
  bits).
